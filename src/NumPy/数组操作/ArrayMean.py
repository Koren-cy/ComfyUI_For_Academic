    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                "计算轴": ("STRING", {
                    "default": "",
                    "tooltip": "沿哪个轴计算平均值，留空表示对所有元素计算，格式如 0 或 (0, 1)"
                }),
                "保持维度": (["是", "否"], {
                    "default": "否",
                    "tooltip": "是否保持原数组的维度"
    ...
    def process(self, 输入数组, 计算轴="", 保持维度="否"):
        try:
            ...
            if 计算轴.strip():
                ...
                result = np.mean(输入数组, axis=axis, keepdims=keepdims)
            else:
                # 对所有元素计算平均值
                result = np.mean(输入数组, keepdims=keepdims)
            
            return (result,)
        except Exception as e:
            raise RuntimeError(f"平均值计算时发生未知错误: {str(e)}")
