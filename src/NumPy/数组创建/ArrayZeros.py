...
class ArrayZeros:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "数据类型": (["float64", "float32", "int64", "int32", "int16", "int8", "uint64", "uint32", "uint16", "uint8", "bool"], {
                    "default": "float64",
                    "tooltip": "数组元素的数据类型"
                }),
            },
    ...
    RETURN_NAMES = ("数组输出",)
    OUTPUT_TOOLTIPS = ("创建的全零数组",)

    def process(self, 形状, 数据类型):
        try:
            shape = self._parse_shape(形状)
            
            zeros_array = np.zeros(shape, dtype=数据类型)
            
            return (zeros_array,)
        except ValueError as e:
            raise ValueError(f"全零数组创建失败: {str(e)}")
        except Exception as e:
            raise RuntimeError(f"创建全零数组时发生未知错误: {str(e)}")
